...
def resolve_indicator(title, indicator_by_key, code=None):
...
def find_matrix_header_row(rows, start_index):
    for index in range(max(0, start_index - 2), min(len(rows), start_index + 1)):
        row = rows[index]
        if normalize(row[5] if len(row) > 5 else "") == AGE_SEX_HEADER_TOKEN:
            return index
    return None
...
def parse_matrix_block(rows, start_index):
    ...
    while row_cursor < len(rows):
        ...
        if kp_cell and normalize(kp_cell) == "total male":
            male_total = first_numeric(row[6:])
            row_cursor += 1
            continue

        if kp_cell and normalize(kp_cell) == "total female":
            female_total = first_numeric(row[6:])
            row_cursor += 1
            continue

        if kp_cell and normalize(kp_cell) in {"sub total", "subtotal", "total"}:
            subtotal_index = 6 + len(age_bands)
            total = parse_number(row[subtotal_index] if len(row) > subtotal_index else None)
            if total is None:
                total = parse_number(row[subtotal_index + 1] if len(row) > subtotal_index + 1 else None)
            row_cursor += 1
            continue
        ...
        if sex == "Male":
            male_total = (male_total or 0) + running_total
        if sex == "Female":
            female_total = (female_total or 0) + running_total
    ...
    male_total = male_total if male_total is not None else 0
    female_total = female_total if female_total is not None else 0
    total = total if total is not None else male_total + female_total
    ...
    return {
        ...
        "is_matrix": True,
        "sub_labels": list(DEFAULT_SUB_LABELS),
    }
            ...
            parsed.append(
                {
                    "code": code,
                    "title": title,
                    "value": matrix_result["value"],
                    "is_matrix": True,
                    "sub_labels": matrix_result["sub_labels"],
                }
            )
...
def update_indicator_disaggregation(indicator, sub_labels, apply_changes):
    ...
    merged = []
    for value in [*existing, *sub_labels]:
        if value and value not in merged:
            merged.append(value)

    changed_fields = []
    if merged != existing:
        indicator.sub_labels = merged
...
def main():
    ...
    missing_sheets = [sheet for sheet in args.sheets if sheet not in workbook.sheetnames]
    if missing_sheets:
        raise SystemExit(f"Sheets not found in workbook: {', '.join(missing_sheets)}")

    indicators = list(Indicator.objects.only("id", "name", "sub_labels"))
    indicator_by_key = {}
    ...
    for sheet_name in args.sheets:
        ...
        for item in parsed_rows:
            ...
            if not args.dry_run:
                ...
                _, project_assignment_created = ProjectIndicator.objects.get_or_create(
                    project=project,
                    indicator=indicator,
                )
            else:
