...
import type { Aggregate, Indicator, IndicatorCategory } from "@/lib/types";
...
function DashboardPageContent({
  ...
  const { data: projectsData, isLoading: projectsLoading } = useAllProjectDetails();
  ...
  const { data: aggregatesData, isLoading: aggregatesLoading, error: aggregatesError } = useAllAggregates({
    ...
    coordinator: selectedCoordinatorId,
    organization: selectedOrganizationId,
    include_org_descendants: selectedOrganizationId ? "true" : undefined,
    date_from: dashboardFilters.dateFrom || undefined,
    date_to: dashboardFilters.dateTo || undefined,
    status: "approved",
  });
  ...
  const widgetAggregateFilters = useMemo(
    () =>
      widgetDialogOpen
        ...
        : null,
    [selectedCoordinatorId, selectedOrganizationId, selectedProjectId, widgetDialogOpen],
  );
  ...
  const descendantsByParent = useMemo(
    () => buildOrganizationDescendantMap(organizations),
    [organizations],
  );
  ...
  const widgetCoordinatorScopedOrganizationIds = useMemo(() => {
    ...
    return new Set([
      widgetCoordinatorId,
      ...(descendantsByParent[widgetCoordinatorId] || []),
    ]);
  }, [descendantsByParent, widgetCoordinatorId]);
  ...
  const allProjects = useMemo(() => projectsData?.results || [], [projectsData]);
    ...
    for (const widget of dashboardPreferences.customWidgets) {
      ...
      if (widgetFilter?.coordinatorId) {
        const coordinatorScope = new Set([
          widgetFilter.coordinatorId,
          ...(descendantsByParent[widgetFilter.coordinatorId] || []),
        ]);
      ...
      if (widgetFilter?.organizationId) {
        const organizationScope = new Set([
          widgetFilter.organizationId,
          ...(descendantsByParent[widgetFilter.organizationId] || []),
        ]);
  ...
  return (
    <div className="mx-auto w-full max-w-[1440px] min-w-0 space-y-6 overflow-x-hidden px-3 pb-8 sm:px-4 lg:px-6">
      ...
      <section className="min-w-0 w-full max-w-full overflow-hidden rounded-2xl border border-border bg-card p-4 shadow-sm sm:p-5">
        <div className="mb-3">
          <div>
            ...
            <p className="mt-1 text-xs text-muted-foreground sm:text-sm">
              Filter the home dashboard by project, coordinator, organization, and reporting period.
            </p>
          </div>
        </div>

<div className="grid w-full min-w-0 grid-cols-1 gap-4 lg:grid-cols-6">          <div className="min-w-0 space-y-1.5">
            ...
            >
              <SelectTrigger id="home-dashboard-project-filter" className="h-11">
                <SelectValue placeholder="All projects" />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="all">All projects</SelectItem>
          ...
          <div className="min-w-0 space-y-1.5">
            ...
            >
              <SelectTrigger id="home-dashboard-coordinator-filter" className="h-10">
                <SelectValue placeholder="All coordinators" />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="all">All coordinators</SelectItem>
