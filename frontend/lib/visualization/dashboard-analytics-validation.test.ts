...
test("dashboard analytics scenarios preserve requested totals and rollups", async (t) => {
...
test("hiv positives target dedupe remains a deliberate v2 compatibility difference", () => {
  ...
  assert.equal(getTargetValue(legacy), 100);
  assert.equal(getTargetValue(compat), 50);
  assert.equal(getAchievementValue(legacy), "18.0%");
