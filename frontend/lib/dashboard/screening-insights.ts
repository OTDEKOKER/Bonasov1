...
function getCanonicalPathwayStageMatches(
  canonical: CanonicalIndicatorDefinition | null,
): Array<{ cardId: string; stageId: string }> {
  if (!canonical) return [];
...
export function buildHomeDashboardScreeningInsights({
  ...
  for (const aggregate of approvedAggregates) {
    ...
    const isHivPreventionMessage = isHivPreventionMessagingIndicator(
      matchText,
      canonicalIndicator,
      indicatorId,
      indicatorCategory,
    ...
    if (canonicalPathwayMatches.length > 0) {
    ...
    } else if (!isMessagingLikeIndicator(matchText, canonicalIndicator)) {
  ...
  const hivPreventionMessageTypeSeries = Array.from(hivPreventionMessageTypeTotals.entries())
    .sort((left, right) => right[1].total - left[1].total)
    .slice(0, 8)
    .map(([key, value], index) => ({
  ...
  const hivPreventionMessageTypeRows =
