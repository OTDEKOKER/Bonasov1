...
function buildPerformanceRows(input: {
  ...
  const descendantsByParent = buildOrganizationDescendantMap(organizations);
  ...
  return targets
    .map((target) => {
      ...
      return {
        ...
        projectName:
          target.project_name ||
          projectsById.get(targetProjectId) ||
...
export function CoordinatorTargetsPage() {
  ...
  const projectsById = useMemo(
    () => new Map(projects.map((project) => [coerceId(project.id), String(project.name || "Project")])),
    [projects],
  );
