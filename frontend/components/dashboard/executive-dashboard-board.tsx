...
export function DashboardExecutiveBoard({
  ...
  const upcomingDeadlines = safeDeadlines.slice(0, deadlineSliceCount);
  const projectOverview = safeActiveProjects.slice(0, 5).map((project) => {
  ...
  const hasHivTestingComparisonData = hivTestingComparison.some(
    (entry) => entry.actual > 0 || entry.target > 0,
  );
  const hasHivMessageTypeData =
    hivPreventionMessageTypeRows.length > 0 &&
    hivPreventionMessageTypeSeries.length > 0;
  const showHivMessageTypePanel =
    screeningInsightsIsLoading || screeningInsightsHasError || hasHivMessageTypeData;
  ...
  const visibleTrendSeries = trendSeries.slice(0, safeChartPreferences.trendSeriesLimit);
                          ...
                          <table className="w-full min-w-[720px] text-sm">
                            ...
                            <tbody className="divide-y divide-border">
                              {safeRecentActivity.length === 0 ? (
                                ...
                                safeRecentActivity.slice(0, 6).map((item, index) => (
                ...
                {showSpotlightPanel ? (
                  <DashboardPanel eyebrow="Spotlight" title="Current focus">
                    {safeActiveProjects.length === 0 ? (
                      ...
                      <div className="space-y-3">
                        {safeActiveProjects.slice(0, 4).map((project) => {
                          const metrics = getProjectMetrics(project);
                          return (
                            <a
                              key={project.id}
