...
export function MessageAnalyticsDashboard({
  hasError,
  isLoading,
  onEditData,
  rows,
    ...
    () =>
      Array.isArray(rows)
        ? rows
            ...
            .filter(
              (row) =>
                normalizeOrganizationLabel(row.cso) !==
                normalizeOrganizationLabel("MAKGABANENG"),
            )
            .map((row) => ({ ...row }))
        : [],
    [rows],
  ...
  const displayMatrix = viewMode === "absolute" ? matrix : viewMode === "percentage" ? percentageMatrix : normalizedMatrix;
