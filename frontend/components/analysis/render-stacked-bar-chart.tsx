...
type StackedBarChartType = BarLikeChart;

function handleSeriesDrilldown(
  chart: StackedBarChartType,
...
export function RenderStackedBarChart(props: {
  chart: StackedBarChartType;
  onDrilldown?: (target: DrilldownTarget) => void;
  ...
  const chartData = chart.data.map((row) => ({
    ...row,
    __chartTotal: chart.series.reduce(
      (sum, series) => sum + Number(row[series.key] ?? 0),
      0,
    ),
  }));
  const finalSeriesKey = chart.series[chart.series.length - 1]?.key;
        ...
        >
          ...
          {chart.series.map((series, seriesIndex) => (
            <Bar
              ...
              barSize={ANALYTICS_CHART_THEME.stackedBarSize}
              radius={
                isHorizontal
            ...
            >
              ...
              {finalSeriesKey === series.key ? (
                <LabelList
                  dataKey="__chartTotal"
